...
function trimString(original, side) {
  if (side === "start") {
    ...
    // Якщо значення аргументу "side" не є "start", "end" або "both", повертаємо повідомлення про помилку
...
function convertCase(original, caseType) {
  if (caseType === "upper") {
    ...
    // Якщо значення аргументу "caseType" не є "upper" або "lower", повертаємо повідомлення про помилку
